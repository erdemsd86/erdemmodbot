...
module.exports = {
conf: {name: 'kadın', aliases: ["k", "woman", "kız"], help: "!kadın @Striga/ID İsim Yaş"},
...
kdb.push(`kullanici.${member.id}.isimler`, {User: member.id, Admin: message.author.id, DisplayName: ServerDisplayName, Role: `<@&${SRol.KadınRol_1}>`, Date: Date.now()})
...
if(isimler.length < 2) return message.channel.send(embed.setDescription(`${member} adlı üyenin ismi "${ServerDisplayName}" olarak güncellendi ve **Kadın** rolleri verildi.`).setColor(client.womanColor())).then(x => x.delete({timeout:15000}), message.react(SEmoji.Onayla)) 

message.channel.send(embed.setDescription(`
${member} adlı üyenin ismi "${ServerDisplayName}" olarak güncellendi ve **Kadın** rolleri verildi.

Kullanıcının üyenin ${isimlerData.length} farklı ismi bulundu.
\`!isimler @EtiketID\` yazarak kontrol edebilirsiniz.`).setColor(client.womanColor())).then(x => x.delete({timeout:15000}), message.react(SEmoji.Onayla)) 
